...
func TestMainFunction(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		for _, url := range urls {
			// Append a new pageText with an empty text field for each URL
			scrapedText = append(scrapedText, pageTextTest{URL: url, Text: ""})
			err := c.Visit(url)
			if err != nil {
		...
		file, err := os.Create("scraped_wikis_test.json")
		if err != nil {
